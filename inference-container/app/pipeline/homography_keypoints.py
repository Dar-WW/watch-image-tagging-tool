...
from .pipeline_result import PipelineResult, KeypointCoords
...
class HomographyKeypointsPipeline(BasePipeline):
    ...
    def _estimate_keypoints_from_obb(
        ...
        logger.info(
            ...
            f"size=({obb_width:.1f}x{obb_height:.1f}), rot={rotation_deg:.1f} deg, "
            f"img={img_w}x{img_h}"
        )
    ...
    def predict(self, image_path: Path, template_name: str = None) -> PipelineResult:
        ...
        if template_name is None:
            try:
                from app.utils.model_mapper import get_template_from_filename
    ...
    def _project_keypoints(
        ...
        Args:
            H: 3x3 homography matrix (Phase1 -> template)
        ...
        logger.info(f"Projecting keypoints: template={template_size}, phase1=({phase1_w}x{phase1_h}), original=({orig_w}x{orig_h})")
